...
export default function Sidebar({ collapsed, onToggleCollapse, email, onSignOut, monthTotal, totalBudgetShort, budgetUsedPct }) {
  return (
    <aside className={`relative hidden lg:flex flex-col h-full bg-pr-navy transition-[width] duration-200 overflow-hidden ${collapsed ? "w-[72px]" : "w-[236px]"}`}>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-pr-default text-[13.5px] font-medium transition-colors cursor-pointer ${collapsed ? "justify-center" : ""} ${
                isActive ? "text-white bg-pr-accent/20 shadow-[inset_3px_0_0_var(--pr-accent)]" : "text-pr-secondary hover:text-white hover:bg-white/5"
              }`
            }
          >
            <Icon size={17} />
      ...
      <button onClick={onToggleCollapse} title={collapsed ? "Expand sidebar" : "Collapse sidebar"} className="absolute top-4 right-[-12px] w-6 h-6 rounded-full bg-pr-navy border border-white/10 text-pr-tertiary hover:text-white flex items-center justify-center cursor-pointer">
